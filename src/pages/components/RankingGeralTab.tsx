...
// ============================================================
// FUNCOES PURAS (reutilizadas/adaptadas do XTreinosTab)
// ============================================================
...
function calcTeamStreak(team: EnrichedTeam): number {
  const dates = team.xtreinos
    .sort((a, b) => a.date.localeCompare(b.date))
    .map((x) => x.date);
  if (dates.length === 0) return 0;
  
  // Simula streak baseado em participações consecutivas (simplificado)
  // No ranking geral, streak = número de xtreinos jogados consecutivamente
  // Como não temos datas globais, usamos o total como aproximação
...
function calcTeamBadges(team: EnrichedTeam): string[] {
  ...
  if (team.top1Count >= 10) badges.push("10x Campeao");
  if ((team.top1Count + team.top2Count + team.top3Count) >= 10) badges.push("Top 3 Regular");
  if (team.xtreinosPlayed >= 10) badges.push("Veterano");
  if (team.xtreinosPlayed >= 20) badges.push("Lenda");
...
export default function RankingGeralTab() {
  ...
  // Enriquecer times com métricas avançadas
  const enrichedRanking: EnrichedTeam[] = useMemo(() => {
    return teamRanking.map((team) => {
      const avgPosition = calcAvgPosition(team as unknown as EnrichedTeam);
      const consistency = calcConsistency(team as unknown as EnrichedTeam);
      const trend = calcTrend(team as unknown as EnrichedTeam);
      const bestPerformance = calcBestPerformance(team as unknown as EnrichedTeam);
      const worstPerformance = calcWorstPerformance(team as unknown as EnrichedTeam);
      const sparkline = calcTeamSparkline(team as unknown as EnrichedTeam);
      const streak = calcTeamStreak(team as unknown as EnrichedTeam);
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center gap-1 text-sm font-medium text-purple-400">
                          {team.xtreinosPlayed >= 10 && <Flame className="w-3.5 h-3.5 text-orange-400" />}
                          {team.xtreinosPlayed}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-center">
